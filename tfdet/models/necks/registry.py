...
def register_neck(fn):
    ...
    _neck_class[model_name] = cls
    _neck_config[model_name] = deepcopy(cfg)
...
def is_necks(model_name):
    """Check if a model name exists"""
    return model_name in _neck_class


def neck_class(model_name):
    """Fetch a model entrypoint for specified model name"""
    return _neck_class[model_name]


def neck_config(model_name):
    """Fetch a model config for specified model name"""
    return _neck_config[model_name]
...
def is_necks_in_modules(model_name, module_names):
...
def is_neck_pretrained(model_name):
